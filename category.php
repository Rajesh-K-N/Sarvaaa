...
if($_SESSION['randnumber']  == $_POST['randnumber'])
{
	if(isset($_POST['submit']))
	{
		if (!empty($_FILES['imge']['name'])) { 
			...
			$updateImageQuery = ", img='$imgname1'"; // Prepare query part to update image
		} else {
			$updateImageQuery = ""; // If no new image is uploaded, do not update the img field
		}
...
?>
		...
		<div id="featured">
			<div class="container">
				<div class="row">
<?php include "leftsidebaradmin.php" ;
?>
    ...
    <td>
        <input type="file" name="imge" id="imge">
        <?php if (!empty($rsedit['img'])) { ?>
            <br>
            <img src="imgcategory/<?php echo $rsedit['img']; ?>" width="100">
        <?php } ?>
    </td>
</tr>
							    <tr>
...
var alphaspaceExp = /^[a-zA-Z\s]+$/; //Variable to validate only alphabets and space
    ...
    // No image selected, but it's optional, so proceed without alert
    return true;
}
	else
	{
