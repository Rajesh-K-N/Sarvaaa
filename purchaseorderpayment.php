...
if ($_SESSION['randnumber'] == $_POST['randnumber']) {
    ...
    if (isset($_POST['submit'])) {
        $purchaseOrderId = mysqli_real_escape_string($con, $_GET['purchaseorderid']);
        $sql = "SELECT * FROM purchase_order WHERE purchase_order_id='$purchaseOrderId'";
        ...
        $sql = "UPDATE purchase_order SET status='Paid' WHERE purchase_order_id='$purchaseOrderId'";
        mysqli_query($con, $sql);

        $sql = "UPDATE product SET quantity= quantity - $rspurchase_order[quantity] WHERE product_id='$rspurchase_order[product_id]'";
        mysqli_query($con, $sql);

        $paymentDescription = "Card Holder name: " . mysqli_real_escape_string($con, $_POST['txtcardholdname']) .
                              " Card Number: " . mysqli_real_escape_string($con, $_POST['txtcardnumb']) .
                              " Expiry date: " . mysqli_real_escape_string($con, $_POST['txtexpirydate']) .
                              " CVV No." . mysqli_real_escape_string($con, $_POST['txtcvv']);
        $paymentType = mysqli_real_escape_string($con, $_POST['paymenttype']);
        $paidAmount = mysqli_real_escape_string($con, $_POST['txtpayment']);
        $dt = date("Y-m-d");

        $sql = "INSERT INTO purchase_order_bill (purchase_order_id, payment_type, payment_description, paid_date, paid_amt, status)
                VALUES ('$purchaseOrderId', '$paymentType', '$paymentDescription', '$dt', '$paidAmount', 'Active')";
...
<div id="featured">
    <div class="container">
        <div class="row">
            <?php include "leftsidebarcustomer.php"; ?>
            <div class="9u">
                <section>
                    <header>
                        ...
                        <table width="477" height="213" border="3">
                            <tbody>
                                <tr>
                                    ...
                                    <td width="308">
                                        <select name="paymenttype" id="select" autofocus>
